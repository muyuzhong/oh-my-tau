...
- **经验怎样被再次使用**：会话、Memory Dream、Skill 与显式 `/learn` 提供不同层次的持久化能力。
...
| 记忆自动整合 | `/dream` 在隔离只读 Agent 中整合当前 Memory 与最近 5 个项目 Session，再由主程序校验并应用变更 | [`dream.py`](lion_code/dream.py)、[`test_dream.py`](tests/test_dream.py) |
...
`/dream` 会读取当前项目 Memory 和最近 5 个项目 Session，并启动一个不共享主对话、只拥有文件读取与搜索工具的隔离 Agent。它可以按需核对当前代码，但不能写项目、执行 Shell、调用 MCP 或启动其他 Agent。

Dream Agent 只返回创建、更新和删除 Memory 的 JSON 计划。主程序随后校验文件名、Memory 类型、内容大小和运行前快照，再集中写入并重建 `MEMORY.md`。这样既能自动合并重复信息和清理过期内容，也不会把模型直接暴露给任意文件写入能力。
...
| Dream 是否直接修改文件 | 只读 Agent 输出计划，主程序校验后应用 | 多一道结构校验，换取路径和删除边界可控 |
| 是否后台自动沉淀 | `/dream` 与 `/learn` 都需要用户显式触发 | 不做后台学习，用户保留最终控制权 |
...
- [ ] **真实后端验证**：`/learn` 与 `/dream` 已有 Mock 与文件写入测试，仍需补充 Anthropic/OpenAI-compatible 的端到端记录；
